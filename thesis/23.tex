...
\section{Результаты исследования устойчивости кольцевой сети нейронов 
...
\begin{equation}\label{mori}
    ...
    |\beta_{jk}|,
\end{equation}
где $\alpha_{jk},\beta_{jk}$ суть элементы матриц $A,B$ соответственно.
...
$n\geqslant3$ и любом $\tau\geqslant 0$.
\end{prop}

Предложение \ref{prop:stab1} также несложно вывести из Теоремы
\ref{th:delayindependentstab} раздела \ref{sec14}
диссертации. 
...
дано в  разделе~\ref{sec26}. Предложение~\ref{prop:stab1} и
Теорема~\ref{th:stab_with_great_n} не дают информации о поведении систем
...
\linebreak $|a|+|b|~>~1$.
Для этого случая мы 
применили исходный код программы <<Устойчивость
нейронных сетей>>.

Здесь мы представляем 
результаты применения программного продукта, Предложения~\ref{prop:stab1} и
Теоремы~\ref{th:stab_with_great_n}. На
рис.~\ref{fig:domain} (а), (б) показаны области
$D_{\tau}$ в плоскости параметров $(a, b)$ модели нейронной сети для некоторых значений~$\tau$. 
\begin{figure}[!ht] 
    \centering
    \subfloat[][]{\includegraphics[width=8cm]{graph1}}\quad 
    \subfloat[][]{\includegraphics[width=8cm]{graph2}}
    \caption[]{Области устойчивости  для системы~\eqref{matrixP},~\eqref{mainmatrix1} (а)
        и системы~\eqref{matrixP},~\eqref{mainmatrix2} (б) с~неограниченным числом нейронов}
	\label{fig:domain}
\end{figure}
...
Область $D_{\tau}$ является расширением области, заданной
неравенством $|a|\hm{+}|b|<1$, на северо-запад и юго-восток до границ, зависящих от
$\tau$. Если точка    $(a, b)$ лежит внутри  $D_{\tau}$ на рис.
\ref{fig:domain}~(а), то система~\eqref{matrixP},~\eqref{mainmatrix1}
асимптотически устойчива. Если  $(a, b)$ лежит вне   $D_{\tau}$ на 
рис.~\ref{fig:domain}~(а), то система~\eqref{matrixP},
\eqref{mainmatrix1} неустойчива при достаточно больших $n$. Аналогичные
утверждения верны для рис.~\ref{fig:domain}~(б) и системы
...
разделе~\ref{sec36}. Таблица~\ref{tab:boundaries} подтверждает оценки
...
неустойчивости исследуемых моделей  в плоскости параметров
(рис.~\ref{fig:domain}) и
...
главе~\ref{chapter3} настоящей диссертации.
