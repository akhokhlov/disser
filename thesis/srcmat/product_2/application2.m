...
label_method2=['Круговое соединение '...
	'нейронов с одинаковым запаздыванием '...
	'взаимодействия с правыми соседями'];
label_model1=['Модель сети: x''j(t) + xj(t) '...
'+ a*xj-1(t) + b*xj+1(t-tau) = 0, 1 <= j <= n'];
label_model2=['Модель сети: x''j(t) + xj(t) '...
	'+ a*xj-1(t-tau) + b*xj+1(t-tau) = 0, '...
	'1 <= j <= n'];
label_intens = ...
	'Интенсивность взаимодействия нейрона с';
...
tip_rand = ['Заполняет входные данные '...
...
label_result_good = ['Уравнение устойчиво '...
	'при tau, находящемся в интервале:'];
...
input_tau = uicontrol(leftPanel1, ...
	'style', 'edit', ...
	'string', num2str(rand(1), 4),...
	'backgroundcolor', 'w',...
	'units', 'normalized', ...
	'position', [0.6 .05 .4 .15]);
...
% Result panel.
resultPanel = uipanel('Parent',rootPanel,...
	'Title',resultPanelTitle,...
	'Position',[0 0 1 .3]);
result_text = uicontrol(resultPanel, ...
	'style', 'text', 'fontsize',fontsize1, ...
		'units', 'normalized', ...
		'position', [0, .5, .7, .5]);
interval_text = uicontrol(resultPanel, ...
	'style', 'text', 'fontsize',fontsize1, ...
		'units', 'normalized', ...
		'position', [0.7, .5, .3, .5]);
% Bottom panel.
...
function calc_callback(~, ~)
	...
	figure('Position', [figure_x figure_y, ...
		figure_width, figure_height],...
		'Name', [label_graph ' ' ...
		num2str(graph_count)],... 
		'NumberTitle', 'off');  
	%determine the algorithm to calculate result
