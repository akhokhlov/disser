...
% pointsPlotter, а также некоторые 
% общие функции (arg, coalition, cone, 
% getIntervals, intersection, isInside),
...
label_error = 'Ошибка';
err_incorrect_cell = ...
	'Вы ввели некорректное значение в ячейке ';
label_graph = 'График';
...
label_method1 = ['Задать матрицы A и B '...
	'с помощью базовой матрицы D'];
label_method2 = ['Задать матрицы A и B ' ...
	'с помощью собственных чисел'];
...
'устойчивости системы, заданной матрицами '...
' A и B'];
label_close = 'Закрыть все графики';
tooltip_close = ...
'Закрывает все ранее открытые графики';
label_result_good = ['Уравнение устойчиво '...
	'при tau, лежащем в следующем '...
	'объединении интервалов:'];
label_result_bad = ...
	'Уравнение неустойчиво при любом tau!';
label_vector = ['Введите коэффициенты в ' ...
'разложении матриц A и B по степеням матрицы D'];
...
% Input panel.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
inputPanel = uipanel('Parent',f,...
'Title',inputPanelTitle,'Position',[0 .3 1 .6]);
sliderPanel = uipanel('Parent',inputPanel, ...
	'bordertype', 'none', 'Position',[0 .9 1 .1]);
uicontrol(sliderPanel, 'style', 'text', ...
	'fontsize', fontsize1,'string',label_dim, ...
...
dimText = uicontrol(sliderPanel,'style','text',...
		...
		'fontsize', fontsize1, ...
		'position', [0.2, 0, .1, 1]);
scale = uipanel('parent', sliderPanel, ...
'bordertype', 'none', 'Position',[.3,0,.7,1]);
	for n=1:5
...
dimSlider =	uicontrol(sliderPanel, ...
	'style', 'slider', 'min', 0, 'max', 4, ...
	'value', 4, 'sliderstep', [.25,.25], ...
	'units','normalized','position',...
	[.3,.5,.7,.5], ...
	'callback', @dimSlider_callback);
	set(dimText,'string',...
get(dimSlider,'value')+1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% input method
...
uicontrol(group, 'style', 'text', ...
	'fontsize', fontsize1, ...
	'string', label_method, ...
	'units','normalized','position',[0,.7,1,.3]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% matrix input
matrixPanel = uipanel('Parent',inputPanel, ...
	'bordertype', 'none',...
	'Position',[0 .25 1 .4]);
dim = str2double(get(dimText, 'string'));
uicontrol(matrixPanel, 'style', 'text', ...
	'fontsize', fontsize1, ...
	'string', inputPanelDesc, ...
	'horizontalalignment', 'right',...
	'units', 'normalized', ...
	'position', [0, 0.6, .2, .4]);
uicontrol(matrixPanel, ...
	'style', 'text', 'fontsize', fontsize1, ...
	'string', label_D, ...
	'horizontalalignment', 'right',...
	'units', 'normalized', ...
	'position', [0.2, 0.6, .1, .4]);
uicontrol(matrixPanel, ...
	'style', 'pushbutton', ...
	'string', label_clear, ...
	'tooltipString', tip_clear,...
	'units', 'normalized', ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Vector Panel
vectorPanel = uipanel('Parent',inputPanel, ...
	'bordertype', 'none',...
	'Position',[0 0 1 .25]);
uicontrol(vectorPanel, 'style', ...
	'text', 'fontsize', fontsize1, ...
	'string', label_vector, ...
	'units', 'normalized', ...
	'position', [0, 0.7, 1, .3]);
uicontrol(vectorPanel, 'style', 'text', ...
	'fontsize', fontsize1, ...
	'string', label_A, ...
	'horizontalalignment', 'right',...
	'units', 'normalized', ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Eig Panel
eigPanel = uipanel('Parent',inputPanel, ...
	'bordertype', 'none',...
	'Position',[1 0 1 .65]);
...
set (eigInput2.root, 'position',[.5,0,.5,1]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Result panel.
resultPanel = uipanel('Parent',f,...
	'Title',resultPanelTitle,...
	'Position',[0 0 1 .3]);
resultChildPanel = uipanel(...
	'Parent',resultPanel,'bordertype', 'none',...
	'Position',[0 0.2 .7 .8]);
resultLabels = zeros (1, MAX_DIM) ;
for n=1:MAX_DIM
	resultLabels(n) = uicontrol(...
		resultChildPanel, 'style', 'text', ...
		'fontsize', fontsize1, ...
		'horizontalalignment', 'left',...
		'units', 'normalized', ...
		'position', [0, 1-.2*n, 1, .2]);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
resultMainPanel=uipanel('Parent',resultPanel,...
	'bordertype', 'none',...
	'Position',[0.7 0.2 .3 .8]);
	result_label=uicontrol(resultMainPanel, ...
		'style', 'text', ...
		'fontsize', fontsize1, ...
		'units', 'normalized', ...
		'position', [0, .5, 1, .5]);
	intervals_label=uicontrol(resultMainPanel, ...
		'style', 'text', ...
		'units', 'normalized', ...
		'fontsize', fontsize1, ...
		'position', [0, 0, 1, .5]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
bottomPanel = uipanel('Parent',resultPanel,...
	'bordertype', 'none',...
	'Position',[0 0 1 .2]);
close_btn = uicontrol(bottomPanel, ...
	'style', 'pushbutton', ...
	'string', label_close, ...
	'tooltipString', tooltip_close, ...
	'units', 'normalized', ...
	'position', [0.8, 0, .2, 1], ...
	'enable', 'off', ...
...
function calc_callback(~, ~)
	...
	if  pos(1) == 0 
		...
		eigD = eig(D) ;
		rowA = getVectorValues(...
			vectorA.children, dim);
		rowB = getVectorValues(...
			vectorB.children, dim);
		for j=1:length(eigD)
			e=[1 eigD(j) eigD(j)^2 eigD(j)^3 ... 
				eigD(j)^4 eigD(j)^4]';
	...
	'Name', [label_graph ' ' ...
	num2str(graph_count)],...  % Title figure
	'NumberTitle', 'off');	  
	[intervals, intervalsCell] = ...
		pointsPlotter(eigA, eigB);
	set (close_btn, 'enable', 'on') ;
	dim = str2double(get(dimText, 'string'));
	for n=1:dim
		set(resultLabels(n), 'string', ...
			buildResult(intervalsCell{n}));
	end
	for n=dim+1:MAX_DIM
		set(resultLabels(n), 'string', '');
	end
...
function dimSlider_callback(~, ~)
	...
	setVectorDim(vectorA.children, ...
		vectorA.children1, val) ;
	setVectorDim(vectorB.children, ...
		vectorB.children1, val) ;
	setEigInputDim(eigInput1, val);
	setEigInputDim(eigInput2, val);
end
% Regenerate matrix.
function random_callback(~, ~)
	setValues(matrixD.children, ...
		rand (getDim())*2-1);
...
%------------ Eig input --------------------%		
function eigInput = ...
		createEigInput(matrixName, eigName)
	root = uipanel();
	uicontrol(root, 'style', 'text', ...
		'fontsize', fontsize1,...
		'string', [label_eig matrixName],...
		'units', 'normalized', ...
		'position', [0, .7, 1,.2]);
	panel = uipanel('parent', root, ...
		'bordertype', 'none',...
		'position', [0, 0, 1, .7]);
	...
	for n=1:MAX_DIM
		strnum = num2str (n) ;
		childrenLabels(n) = uicontrol(panel, ...
			'style', 'text', 'fontsize', fontsize1,...
			'horizontalAlignment', 'right', ...
			'string', [eigName strnum ' = '],...
			'units', 'normalized', ...
			'position', [0, 1-.2*n, .3, .2]);
		children(n) = uicontrol(panel, ...
			'style', 'edit', ...
			'string', num2str (rand (1)*2-1, 4), ...
			'backgroundcolor', 'w',...
			'units', 'normalized', ...
			'position', [.3, 1-.2*n, .25, .2]);
		childrenLabels1(n) = uicontrol(panel, ...
			'style', 'text', 'fontsize', fontsize1,...
			'horizontalAlignment', 'center', ...
			'string', ' + ',...
			'units', 'normalized', ...
			'position', [0.55, 1-.2*n, .1, .2]);
		children1(n) = uicontrol(panel, ...
			'style', 'edit', ...
			'string',num2str(rand(1)*2-1,4), ...
			'backgroundcolor', 'w',...
			'units', 'normalized', ...
			'position', [.65, 1-.2*n, .25, .2]);
		childrenLabels2(n) = uicontrol(panel, ....
			'style', 'text', 'fontsize', fontsize1,...
			'horizontalAlignment', 'left', ...
			'string', '*i',...
			'units', 'normalized', ...
			'position', [0.9, 1-.2*n, .1, .2]);
	end
	eigInput.root = root;
...
function setEigInputDim(eigInput, dim)
	for n = 1:dim
		set(eigInput.children(n), ...
			'visible', 'on');
		set(eigInput.children1(n), ...
			'visible', 'on');
		set(eigInput.childrenLabels(n), ...
			'visible', 'on');
		set(eigInput.childrenLabels1(n), ...
			'visible', 'on');
		set(eigInput.childrenLabels2(n), ...
			'visible', 'on');
	end
	for n = dim+1:MAX_DIM
		set(eigInput.children(n), ...
...
function vector = getEigValues(eigInput)
	...
	for j=1:getDim()
		...
		if length(parseValueRe) ~= 1 || ...
				length(parseValueIm) ~= 1
			str = [err_incorrect_cell '(' ...
				num2str(j) ').'];
...
function vector = createVector(dim)
	...
	strings = {'E + ', 'D + ', 'D^2+', ...
		'D^3+', 'D^4+', 'D^5'};
	for j=1:dim
		children(j) = uicontrol(root, ...
			'style', 'edit', ...
			'string', num2str(rand(1)*2-1,4),...
			'backgroundcolor', 'w',...
			'units', 'normalized', ...
			'position',[(j-1)/dim,0,1/dim/2,1]);
		children1(j) = uicontrol(root, ...
			'style','text','string',strings(j),...
			'fontsize', fontsize1, ...
			'units', 'normalized', ...
			'position',[(j-1)/dim+1/dim/2, ...
			0, 1/dim/2, 1]);
	end
	vector.root = root;
...
function matrix = createTable(rows, cols)
	...
	for i=1:rows
		for j=1:cols
			children(i,j) = uicontrol(root, ...
				'style', 'edit', ...
				'string',num2str(rand(1)*2-1,4),...
				'backgroundcolor', 'w',...
				'units', 'normalized', ...
				'position', [(i-1)/rows, ...
				1-j/cols, 1/cols, 1/rows]);
		end
	end
	matrix.root = root;
...
function setValues(children, matrix)
	for i=1:length(matrix)
		for j=1:length(matrix)
			if i <= length(children) && j <= ...
					length(children)
				set(children(i,j), 'string', ...
					num2str (matrix(i,j), 4));
			end
		end
	end
end
