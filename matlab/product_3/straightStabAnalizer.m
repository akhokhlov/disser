%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Проверяет данную точку на устойчивость 
% непосредственно, используя годограф.
% type - тип соединения нейронов 
% (1 для несимм., 2 для симм. взаимодействия),
...
function [stab] = straightStabAnalizer(...
    ...
    if type == 1    % первый тип соединения. 
    ...
    % Находим w, которая является параметром 
    % для точки первого пересечения
    % годографа с осью 0a.
    PI=2*pi;
    w=0:PI/100:PI;
    maxW = zeros(1, number) ;
    ...
    % Для каждой из точек Mj срезаем конус 
    % на высоте её третьей компоненты
    % (переводим задачу на плоскость), 
    % затем определяем, лежит ли эта точка
    ...
    for k=j
        ...
        % Находим расстояние от каждой точки 
        ...
        % точки овала, то исследуемая пара 
        % значения a, b является неустойчивой 
    ...
    % Если протестированы все точки Mj и 
    % среди них не оказалось неустойчивых, 
    % то исследуемая пара значения a, b 
    % является устойчивой.
    stab = 1;
end
